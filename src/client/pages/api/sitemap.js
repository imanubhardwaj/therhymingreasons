...
export default async (req, res) => {
    ...
    poems.forEach(poem => {
        doc.ele("url")
            .ele("loc").txt(`${process.env.BASE_URL}/posts/${poem.slug}`).up()
            .ele("priority").txt(poem.indexPriority.toFixed(1)).up()
            .ele("lastmod").txt(formatDate(new Date(poem.publishDate))).up()
